PROTEINTENSOR SOFTWARE EVALUATION AGREEMENT

This Evaluation Agreement ("Agreement") is entered into between Clay Moore (claytonwaynemoore@gmail.com)
("Licensor") and the individual or organization named below ("Evaluator").

Evaluator name / organization: ___________________________________
Effective date: ___________________________________

1. LICENSE GRANT
   Licensor grants Evaluator a non-exclusive, non-transferable, royalty-free
   license to access and evaluate the ProteinTensor software ("Software") solely
   for internal, non-commercial evaluation purposes for 30 days from the
   Effective Date ("Evaluation Period").

2. RESTRICTIONS
   Evaluator shall not:
   (a) use the Software in production systems or for commercial purposes;
   (b) copy, modify, distribute, sublicense, or create derivative works;
   (c) reverse engineer or attempt to derive the source code beyond what is
       provided;
   (d) disclose the Software or its contents to any third party without prior
       written consent from Licensor.

3. OWNERSHIP
   The Software and all intellectual property rights therein remain the sole
   property of Clay Moore (claytonwaynemoore@gmail.com). This Agreement does not transfer any ownership rights.

4. CONFIDENTIALITY
   Evaluator agrees to keep the Software and any related technical information
   strictly confidential and to use the same degree of care as it uses to
   protect its own confidential information, but no less than reasonable care.

5. NO WARRANTY
   The Software is provided "as is" for evaluation purposes only, without any
   warranty of any kind.

6. TERMINATION
   This Agreement terminates automatically at the end of the Evaluation Period
   or upon written notice by either party. Upon termination, Evaluator shall
   destroy or return all copies of the Software.

7. GOVERNING LAW
   This Agreement shall be governed by the laws of [your state].

Evaluator signature: _____________________ Date: _______________
Printed name / title: _____________________________________

Licensor: Clay Moore (claytonwaynemoore@gmail.com)    Date: _______________
